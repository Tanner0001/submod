...
| 49th Parallel Escalation (§3) | **Considered done.** Tension tracking via `on_state_control_changed` + decay pulse is functional. Sabotage applies a real dynamic state modifier (HOI4's Operations system can't target unoccupied hostile states, so this is the verified substitute). Border Guard Deployment no longer grants `CAN_refugee_crisis_help` - the base mod already grants that idea through its own event chain on the real refugee-crisis mechanic, so this was a duplicate and has been removed; the decision is now just its PP cost/flavor. Intervention issues a real `create_wargoal` with `type = topple_government` (confirmed real, used by several KR ideological-intervention focuses) instead of the too-extreme `annex_everything` placeholder. |
