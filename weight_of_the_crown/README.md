...
| Dominion Solidarity (new - Canada helps RAJ against HND) | Canada-scoped decisions (`wotc_raj_dispatch_advisors` -> `wotc_raj_equipment_shipment`/`wotc_raj_joint_offensive`) targeting RAJ, mirroring Operation Anvil's shape. Verified against RAJ's own files: it starts in a separate faction with PRF as an ally (confirming PRF isn't a target here), has a real focus path into the Entente (`ENT = { add_to_faction = RAJ }`), and a real war path against HND (`will_lead_to_war_with = HND`) - both gates are achievable in an actual game. |
