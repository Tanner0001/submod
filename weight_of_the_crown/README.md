...
| Operation Homecoming (§4) | Phase gating via flags is functional. Phase 1 now actually annexes Iceland (state 100, its entire territory - confirmed a single-state minor) via `transfer_state` and builds up its existing naval base, per user confirmation that full annexation is intended. Gated on state 100's current owner, not `country_exists = ICE`, so it still works if Iceland is conquered by someone else (e.g. Scandinavia) before Canada gets to it - checked Iceland's own focus tree, joining a Nordic faction doesn't transfer its territory, but ordinary wartime conquest by anyone else could. Phase 2/3 state-targeted bonuses are still stubbed at country scope. |
