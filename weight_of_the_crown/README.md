...
| 49th Parallel Escalation (§3) | **Considered done.** Tension tracking via `on_state_control_changed` + decay pulse is functional. Sabotage applies a real dynamic state modifier (HOI4's Operations system can't target unoccupied hostile states, so this is the verified substitute). Border Guard Deployment no longer grants `CAN_refugee_crisis_help` (duplicate of a real base-mod effect, removed) and is now one-time only: it spawns 3 divisions under a **genuinely new** "RCMP Emergency Response Team" division template (built from `ranger_battalion`, real and special-forces-flagged), created purely via a `division_template = { ... }` effect (`wotc_initialize_rcmp_ert_template` in `wotc_effects.txt`) - the exact same real pattern the base game itself uses to build its vanilla "Rangers" template from `rangers_tech`'s `on_research_complete`, confirmed in `common/scripted_effects/00_initialize_templates_from_techs.txt`, so no shadow-copy of `history/units/CAN.txt` was needed. Since `ranger_battalion` is gated behind the "No Compromise, No Surrender" DLC (`has_dlc_ncns`) and the `rangers_tech` tech, the decision force-researches that tech via `set_technology` for anyone who owns the DLC; players without the DLC (a hard, unscriptable limit) get a fallback deployment using Canada's existing "District Militia" template instead of a silent failure. Divisions garrison at Toronto (state 276), Quebec/Montreal (state 832), and Winnipeg (state 467) - all verified real locations, not guessed. Intervention issues a real `create_wargoal` with `type = topple_government` (confirmed real, used by several KR ideological-intervention focuses) instead of the too-extreme `annex_everything` placeholder. |
| Operation Homecoming (§4, redesigned) | **Bug fixed (2026-09-15, from in-game report - "Overture to Reykjavik" always showed a red X on its Iceland prerequisite no matter what):** all four Iceland/Faroe ownership checks used `owner = CAN` inside a state scope, which isn't a real trigger anywhere in this repo or the base game (`owner =` is a history-file key). Replaced with the real trigger, `is_owned_by = CAN` (confirmed via `events/wotc_canada.txt`'s own usage and many base-game events), across `wotc_ice_overture`, `wotc_ice_occupy`, `wotc_faroe_overture`, and `wotc_faroe_annex`. **Considered done.** Iceland (state 100) is a two-step overture-then-occupy chain (45-day buildup). Faroe Islands (state 337, DEN) gets its own overture-then-annex chain, gated on Iceland. Orkney & Shetland (state 938, ENG - already has `add_claim_by = IMP` and a level-8 naval base + supply hub) is a real invasion objective, not annexed by decision - activates once Canada controls it via `is_controlled_by = CAN`. The landing requires both secured, and its bonus now ramps: `wotc_idea_homecoming_landing_bonus_peak` (amphibious_invasion 0.30) for days 1-4, swapped to the lighter sustain idea (0.15) for days 5-14 via `events/wotc_canada.txt`'s wotc_canada.20/.21 pair, then removed. Every modifier key this system uses (`intel_network_gain`, `special_forces_cap`, `amphibious_invasion`, `supply_consumption_factor`) is now cross-checked against real usage elsewhere in this repo, not guessed. |
